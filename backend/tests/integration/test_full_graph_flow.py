...
async def test_full_flow_completes_under_ten_seconds():
    ...
    state["fail_count"] = 0
    state["current_level_index"] = 0
    # Clear stale level-0 entries from the 3 pre-sublevel fails so the passing
    # attempt registers as attempt #1 and earns the first_attempt_pass bonus.
    # This matches real behaviour: sublevel completion resets the attempt slate.
    state["test_history"] = [t for t in state["test_history"] if t["level_index"] != 0]
    state = await sim_level_gate(state, score=0.85)
    state = await sim_gamification(state)

    elapsed = time.monotonic() - start
    assert elapsed < 10.0, f"Integration test took {elapsed:.2f}s — real I/O may be leaking in"
    assert "Bronze" not in state["badges"]  # only level 0 completed — Bronze not yet
    assert state["points"] == 50 + 25      # level_complete + first_attempt_pass (exactly)
